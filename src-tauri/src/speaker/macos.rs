...
pub fn get_output_sample_rate(device_id: Option<&str>) -> Result<u32> {
    if let Some(device_id) = device_id.filter(|value| !value.is_empty()) {
        for device in ca::System::devices()?.iter() {
            if device.uid()?.to_string() == device_id {
                return Ok(device.actual_sample_rate()? as u32);
            }
        }
    }
    Ok(ca::System::default_output_device()?.actual_sample_rate()? as u32)
}
...
impl SpeakerInput {
    pub fn new(_device_id: Option<String>) -> Result<Self> {
        // A physical output sub-device supplies the aggregate device's clock.
        // A tap-only aggregate can start successfully but later stop producing
        // input frames, especially with virtual outputs such as eqMac/Teams.
