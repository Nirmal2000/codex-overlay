...
pub fn run() {
    ...
    let mut builder = tauri::Builder::default()
        ...
        .manage(passive_input::PassiveInputState::default())
        .manage(microphone::MicrophoneState::default())
        .manage(echo::EchoCancellationState::default())
        .manage(realtime::RealtimeState::default())
        .manage(remote::RemoteBridgeState::default())
        .manage(transcribe::TranscriptionState::default())
    ...
    let mut builder = builder
        .invoke_handler(tauri::generate_handler![
            ...
            realtime::send_realtime_turn,
            realtime::retry_realtime_turn,
            realtime::append_realtime_verified_answer,
            realtime::close_realtime_session,
            remote::start_remote_bridge,
            remote::get_remote_bridge_info,
            remote::publish_remote_snapshot,
            passive_input::set_passive_capture,
            passive_input::get_passive_capture,
            passive_input::get_passive_input_ready,
            microphone::start_microphone_capture,
            microphone::stop_microphone_capture,
            microphone::get_microphone_devices,
        ...
        .setup(|app| {
            ...
            if std::env::var("CODEX_OVERLAY_DIAGNOSTIC").as_deref() == Ok("1") {
                if let Some(window) = app.get_webview_window("main") {
                    let _ = window.set_content_protected(false);
                }
            }
            passive_input::start_listener(app.handle().clone());
            #[cfg(desktop)]
            {
