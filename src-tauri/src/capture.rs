...
#[tauri::command]
pub async fn capture_screen_file(app: tauri::AppHandle) -> Result<ScreenshotFile, String> {
    let dir = app
        .path()
        .app_data_dir()
    ...
    let status = tokio::process::Command::new("/usr/sbin/screencapture")
        ...
        .status()
        .await
        .map_err(|e| format!("Failed to start macOS screen capture: {e}"))?;
    if !status.success() {
        return Err("macOS screen capture failed".into());
    }
    let optimize_path = path.clone();
    tokio::task::spawn_blocking(move || optimize_screenshot(&optimize_path))
        .await
        .map_err(|error| format!("Screenshot optimization task failed: {error}"))??;
    Ok(ScreenshotFile {
        path: path.to_string_lossy().into_owned(),
    })
}

fn optimize_screenshot(path: &std::path::Path) -> Result<(), String> {
    const MAX_EDGE: u32 = 1920;
    let image =
        image::open(path).map_err(|error| format!("Failed to decode screenshot: {error}"))?;
    ...
    resized
        .save(path)
        .map_err(|error| format!("Failed to write optimized screenshot: {error}"))
}

#[tauri::command]
pub fn save_screenshot(app: tauri::AppHandle, base64_png: String) -> Result<String, String> {
    ...
    std::fs::create_dir_all(&dir)
        .map_err(|e| format!("Failed to create screenshot directory: {e}"))?;
    let path = dir.join(format!("{}.png", uuid::Uuid::new_v4()));
    std::fs::write(&path, bytes).map_err(|e| format!("Failed to save screenshot: {e}"))?;
